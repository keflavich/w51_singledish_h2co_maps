...
for meastype in ('best','mean'):
    ...
    for ii,vrange in enumerate((vrange1,vrange2)):
        ...
        makefig(hdu1, 4+2*ii, name='weighted_mean_{2}fit_density_v{0}to{1}'.format(vrange[0].value, vrange[1].value, meastype), vmax_lin=100)
        makefig(hdu2, 5+2*ii, name='masked_weighted_mean_{2}fit_density_v{0}to{1}'.format(vrange[0].value, vrange[1].value, meastype), vmax_lin=100)
