...
yielding a factor of 2 error in derived density \citep{Zeiger2012a}, but the
...
%\added{As long as we can assume the
...
The second histogram in Figure \ref{fig:abundances} shows the abundance
derived by comparing the integrated \ortho column to the integrated \thirteenco
column using an assumed $^{12}$C/$^{13}$C ratio 60 and $\twelveco$ abundance
$10^{-4}$ \hh.  
The distributions are offset from one another by about 1.5 orders of magnitude,
indicating that these abundances are not very reliable.  The \thirteenco-derived
column may be underestimated or the \ortho column substantially overestimated.
Nonetheless, the \ortho abundances are strongly constrained to the range
$[10^{-10.5},
10^{-6.5}]$.
}
...
{figures/H2CO_abundance_fromratioto13CO.pdf}
{\added{(a) A histogram of the abundances derived for each spatial pixel using
...
10^{-9.9\pm0.6}$.
(b) A histogram of the abundances derived by dividing the integrated
$N(\ortho)/N(\thirteenCO)$ using an assumed \thirteenco abundance $X^{13} =
X^{12}/60 = 1.7\ee{-6}$.  The overlaid fits show that 75\% of the area
of W51 is consistent with abundance $X(\ortho) = 10^{-8.4\pm0.3}$ and
25\% is consistent with $X(\ortho) = 10^{-7.7 \pm 0.7}$.
}
}
{fig:abundances}{0.5}{6in}
