...
The data set has been made public \todo{if someone is willing to host. This may be CfA dataverse.}
...
We present maps of the local volume density of the molecular gas.
...
The W51 cloud complex (Figure \ref{fig:w51large}) is located at $\ell\sim49,
b\sim-0.3$, very near the Galactic midplane at a distance of 5.1 kpc
...
% \todo{[May be removed]}
The \formaldehyde line ratio can be directly transformed into a volume
...
The column density is degenerate the velocity gradient in LVG models.  The \hh
column density is degenerate with this gradient \emph{and} the abundance of
\ortho.  Precise measurements of the \formaldehyde abundance are not generally
...
Ao2013a} and found to be consistent with the observations.  Nonetheless, little
is known about local variations in \ortho abundance, except that it freezes out
in cold, dense cores \citep{Young2004a}.
...
but in front of the 68 \kms component.}
...
We detect \formaldehyde at $\sim54$ and $\sim66$ \kms toward G49.10-0.40, consistent
with it being behind the molecular cloud complex.  It also has an extreme RRL
velocity, $V_{lsr} \approx 72\kms$, the most redshifted seen in the entire W51
region (see Figures \ref{fig:kinematics} and \ref{fig:hiirrlspec}).
...
associated with the W51 B cloud complex.  The Galactic rotation curve doesn't
allow for velocities red of $\sim60$ \kms, and almost none of the molecular gas
exceeds $\sim70$ \kms even on the wings.  The \hii regions are therefore
probably shooting out the back side of the molecular cloud, perhaps
accelerating ionized gas from the $\sim 66$ \kms component further to the red.
...
\subsubsection{W51 Main \& W51 IRS 2}
...
The \formaldehyde spectra are deepest at 68 \kms, but they are too deep to be
entirely behind the \hii regions.  This indicates that the 55 \kms ionized gas
must be embedded within the molecular cloud, with molecular gas on \emph{both}
sides of the ionized gas along the line of sight.
...
behind nearly all of the \hii regions in the W51 complex.  There are 40 \kms
clouds clearly seen in \hi absorption \citep{Stil2006a} that are not associated
with these molecular clouds.
...
\citet{Ginsburg2011a}.  We adopt the `best' model fit to the line ratio in each
voxel as the point in the full 4D parameter space where $\chi^2$ is minimized.
We have therefore not enforced any constraints on the column density,
temperature, or ortho-to-para ratio when fitting.

The column-weighted volume-density along each line of sight is shown in Figures
\ref{fig:wtdmeandens} and \ref{fig:wtdmeandensvrange}.  The former shows the
weighted density over all voxels and the latter shows the weighted density over
the two velocity ranges previously discussed.

The overall picture is of a central proto-cluster region with most of the gas
mass at a density $n\sim10^6$ \percc within a diameter of $\sim3$ pc, surrounded
by a rich cloud in which most of the mass is at a density $\sim10^5$ \percc out
to a diameter $d\sim14$ pc.


% parplots_chi2gridfit.py
\Figure
{figures/H2CO_ParameterFitPlot_weighted_mean_bestfit_density_linear.pdf}
...
% parplots_chi2gridfit.py
\FigureTwoAA
{figures/H2CO_ParameterFitPlot_weighted_mean_bestfit_density_v40.0to62.0_linear.pdf}
{figures/H2CO_ParameterFitPlot_weighted_mean_bestfit_density_v62.0to75.0_linear.pdf}
...
presented as evidence for a cloud-cloud collision \citep{Kang2010a}.  Examination of
the \formaldehyde line ratios has shown that the protocluster is embedded in the $\sim55$ \kms
molecular cloud.  The high density of the 68 \kms cloud at this position indicates that it is
likely interacting...

\todo{Is the W51 filament like Nessie?  Too coherent over large scales, flattened in the midplane?}
