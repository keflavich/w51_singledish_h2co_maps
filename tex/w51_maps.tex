...
%\todo{This paragraph may need expansion or removal.  See Section \ref{sec:futureev}}
Star formation appears to occur only above a set volume density threshold in
molecular gas, specifically $n(\hh)\gtrsim10^4$ \percc \citep[][, who advocate
a column density threshold corresponding to this density]{Lada2010a}.  This
threshold is contentious \citep{Burkert2012a,Clark2013b}, but it can only be
evaluated when measurements of density and column density are simultaneously
available.
...
Each spectrum was calibrated using a noise calibration diode as
...
mostly uniform background of diffuse emission.
...
The Arecibo data appear to have smaller systematic continuum offsets than the
GBT data, but they are more visually pronounced because there is much more
diffuse emission at 6 cm.  The continuum zero-point level in the Arecibo data
was set to be the 10th percentile value of each scan, which is effectively the
minimum value across each scan but with added robustness against
noise-generated false minima.  In the eastmost and westmost blocks, this
strategy was very effective, as there are clearly areas in each scan that see
no continuum.  However, in the central block, this approach resulted in a
...
in the areas that they overlap, it is clear that the continuum is
reliable above $\gtrsim0.5$ K, which is the entire regime in which it is a
significant contributor to the total background emission (at lower levels, the
CMB is dominant).


\FigureTwoAA{figures/continuum_11.pdf}
            {figures/continuum_22.pdf}
{Continuum images of the 6 cm Arecibo data \todo{(left or top)} and 2 cm GBT
data \todo{(right or bottom)}} {fig:continuum}{1.0}{6in}
...
The abundance is left as a free parameter in the model fitting.
...
\citet{Ginsburg2011a}.  We measure $\chi^2$ over the full 4D parameter space
(density, column density, gas temperature,
...
W51 main.  The extracted region is identified in purple in Figure
\ref{fig:filament_pvslice}.}
...
This analysis relies on the \thirteenco being optically thin and thermally
...
feedback mechanisms in parallel.  It may also be a good location to examine how
star formation comes to an end in the presence of massive star feedback.
...
An alternative interpretation of the difference between the Galactic and
extragalactic \formaldehyde is that the \hii regions still form in these
galaxies, but their luminosity is significantly suppressed.  The most
straightforward explanation for a lower luminosity is that they are smaller,
perhaps compressed by a much more massive and overpressured ISM.  Tightly
squeezed \hii regions such as these are locations where radiation pressure may
eventually become dominant, with multiple photon scatterings transferring
additional momentum to the surrounding medium as described in
\citet{Murray2010b}; this scenario does not occur in the optically thin \hii
