...
the \oneone to the \twotwo optical depth.  They are masked to show significan
...
However, there are some clear outliers within the map: the clouds at G49.0-0.3
...
The region 49.47-0.42 also appears to have a very low ratio.  In this region,
...
is nonphysical.  However, when comparing the optical depth of the two line components
directly, the deeper \twotwo component remains peculiar: the simple RADEX LVG
models cannot accomodate this deeper \twotwo line with any combination of
density, temperature, column density, and ortho-to-para ratio.
...
the data.
The data are masked with a filter described in Section \ref{sec:h2co} and
cover the range $75 > V_{LSR} > 40$ \kms; see Figures \ref{fig:lowerpeakoptdepth}
and \ref{fig:upperpeakoptdepth} for individual velocity components.
In general, lower (redder) ratios in figures (c) and (d) indicate higher
densities, however in the filament at 49.0-0.3, the low ratio is due to the
geometry in which $T_{continuum}$ is in the \emph{foreground} of the molecular
gas.}
{fig:peakoptdepth}
...
{Same as Figure \ref{fig:peakoptdepth}, but limited to $65 > V_{LSR} > 40$ \kms.}
...
{Same as Figure \ref{fig:peakoptdepth}, but limited to $75 > V_{LSR} > 65$ \kms.}
