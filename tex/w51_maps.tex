...
%, suggesting that its star-forming lifetime has come to an
...
%\todo{Compiled on \today\ at \currenttime.}
...
column density threshold corresponding to this density]{Lada2010a}; \added{note however
that this is a smooth threshold, not  a step function \citep{Padoan2013a}}.  The `dense
...
%to be taken, but not the corresponding 0.2 \kms data.}
...
suggested that we did not detect any emission.}
% 14488.479,13036.06,14128.61,14825.79
...
minimum $\chi^2$ value.
%\todo{weighted by their
%relative likelihood $\mathcal{L}_{rel} = e^{\chi^2/2}$ over the full parameter
%space.}
...
The overall picture is of a central protocluster region (W51 Main and IRS2) with most
...
mass-weighted\footnote{\added{This is a simplification; other factors in the weight
include the optical depth for an optically thick line, the abundance of \formaldehyde,
and the geometry if there is a background source.  For most of the W51 cloud, a
fixed abundance and optically thin lines are not a bad assumption, and we account
for the geometry in the discussion.}}
measurement of
...
the overall uncertainty.  \added{As long as we can assume the
\thirteenco-traced gas represents the majority of the total cloud gas, the
$^{13}$C/$^{12}$C ratio has no effect on our measurements.} 
...
% These PPs are not obviously relevant
...
is consistent with the 5-20\arcmin scales required to recover a star formation `law'
from Galactic plane survey observations \citep{Vutisalchavakul2014a}.  The
improved correlation at the highest densities implies that the massive star
formation threshold is closer to $10^5$ \percc than the $10^4$ \percc often
measured for nearby, low-mass star-forming regions.}
...
%The broad-brush picture is of a massive protocluster region with high density
...
%chaff with relatively little star formation.  Much of this low-density material
%is being disrupted by feedback from massive stars in the form of ionized gas
...
anticorrelated with the dense gas.  The density threshold for high-mass star
formation is higher than that for low-mass star formation.}
...
with Jonathan Tan and Neal Evans.
