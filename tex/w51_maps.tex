...
as $X = n(\hh)/N(\ortho) \left(\frac{1 \kms}{1 \kms \perpc}\right)$.
...
abundance distribution: 70\% of the area is consistent with an abundance
$X(\ortho) = 10^{-8.5 \pm 0.6}$ and 30\% with $X(\ortho) = 10^{-10.0\pm0.5}$.
However, we caution that these abundances are sensitive to the background
brightness and are best treated as order-of-magnitude estimates.
}

\Figure
{figures/H2CO_ParameterFits_mean_likewtd_abundance.pdf}
{\added{A histogram of the abundances derived for each spatial pixel using the
LVG grid fit.  The abundance shown assumes a velocity gradient 1 \kms \perpc.
The overlaid fits show that 70\% of the area of W51 is consistent with an
abundance $X(\ortho) = 10^{-8.5 \pm 0.6}$ and 30\% with $X(\ortho) =
10^{-10.0\pm0.5}$.}
}
