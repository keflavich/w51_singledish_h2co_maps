...
cubefiles = ('h2co_singledish/W51_H2CO11_taucube_supersampled.fits',
             ...
             'grs-50-cube_supersampledh2cogrid.fits',
...
for jj,color in enumerate(('green','red','blue','cyan','yellow')):
    ...
    for ii,cubefn in enumerate(cubefiles):
        ...
        title = titles[ii]
        header = pvextractor.utils.flatten_header(fits.getheader(cubefn))
        ww = wcs.WCS(header)
        ...
        if cubefn in pvs: # caching
            ...
            pv, npv = pvextractor.pvdiagram(cube,endpoints)
            pvs[cubefn] = (pv,npv,velo,cdelt)
        ...
        aspect = float(pv.shape[1])/float(vmask.sum()) * cdelt / ((vmax-vmin)/float(vmask.sum()))
        ...
        pl.imshow(pvtoshow,
                  extent=[pv.shape[1]*cdelt,0,vmin,vmax],
                  aspect=aspect,
                  cmap=cm,
                  vmin=min_val,
                  vmax=max_val,
        ...
        pl.vlines(np.cumsum(npv)*cdelt,vmin,vmax,color='b',linestyle='--',linewidth=2,alpha=0.5)
        pl.hlines(65,0,pv.shape[1]*cdelt, color='b', linestyle=':',alpha=0.5)
        pl.annotate(title,(0.05,0.9),xycoords='axes fraction',color='k')

    colorpvs[color] = pvs

    # regrid tau22 onto tau11 after smoothing along the velo axis
    #pv1,npv1,velo1,cdelt1 = pvs['h2co_singledish/W51_H2CO11_taucube_supersampled.fits']
    #pv2,npv2,velo2,cdelt2 = pvs['h2co_singledish/W51_H2CO22_pyproc_taucube_lores_supersampled.fits']
    ...
    pv2toshow = pv2[vmask2[::index_order],:]
    ...
    for kk in [1,2,3,4,5,6]:
        pl.subplot(nxsp,nysp,kk).xaxis.set_ticklabels([])
    for kk in xrange(1,nxsp*nysp+1):
        pl.subplot(nxsp,nysp,kk).axis([0,pv.shape[1]*cdelt,vmin,vmax])
    for kk in [7,8,9]:
        pl.subplot(nxsp,nysp,kk).set_xlabel("Offset (\")",fontsize=18)
    for kk in [1,4,7]:
        pl.subplot(nxsp,nysp,kk).set_ylabel("V$_{LSR}$ (km/s)",fontsize=18)
    pl.savefig('W51_FilamentPVDiagrams_co32included_regrid_%s.png' % color, bbox_inches='tight')
    pl.savefig('W51_FilamentPVDiagrams_co32included_regrid_%s.pdf' % color, bbox_inches='tight')
