...
con = ax.contour(hdus[0].data, levels=[-0.1,0.025,0.05,0.1,0.2,0.4])
...
fig11.savefig(os.path.join(paths.figurepath, "filaments_H2CO_13CO_density_pvslice.pdf"))
A = matplotlib.axes.Axes(fig11, [0.9,0.1,0.025,0.25])
pl.colorbar(c, cax=A)
fig11.add_axes(A)
fig11.savefig(os.path.join(paths.figurepath, "filaments_H2CO_13CO_density_pvslice_colorbar.pdf"))
...
fig13 = pl.figure(13)
fig13.clf()
F = aplpy.FITSFigure(bgps, convention='calabretta', figure=fig13)
...
polygons = pvextractor.geometry.path.Path(endpoints, width=25*u.arcsec).sample_polygons(1, wcs=bgps_wcs)
...
F.save(os.path.join(paths.figurepath, 'filament_extraction_region_on_BGPS.pdf'))
